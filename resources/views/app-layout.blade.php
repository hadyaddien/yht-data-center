...
<body class="h-full bg-[#f1f4f8]">
        ...
        <nav class="flex-1 py-3 overflow-y-auto">
            @php
                $currentRoute = match (true) {
                    ...
                    request()->routeIs('cetak-laporan*') => 'cetak-laporan',
            ...
            <a href="{{ route('dashboard') }}"
                class="sidebar-item flex items-center gap-3 px-4 py-2.5 text-[#c5d5e8] text-sm font-medium transition-all mx-2 rounded-lg mb-0.5 {{ $currentRoute === 'dashboard' ? 'active' : '' }}">
            ...
            <a href="{{ route('sekolah.index') }}"
                class="sidebar-item flex items-center gap-3 px-4 py-2.5 text-[#c5d5e8] text-sm font-medium transition-all mx-2 rounded-lg mb-0.5 {{ $currentRoute === 'sekolah' ? 'active' : '' }}">
                <svg class="w-4.5 h-4.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <a href="{{ route('cetak-laporan.index') }}"
                class="sidebar-item flex items-center gap-3 px-4 py-2.5 text-[#c5d5e8] text-sm font-medium transition-all mx-2 rounded-lg mb-0.5 {{ $currentRoute === 'cetak-laporan' ? 'active' : '' }}">
                <svg class="w-4.5 h-4.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            @if (auth()->user()->isSuperAdmin() || auth()->user()->isAdminWilayah())
                ...
                <a href="{{ route('users.index') }}"
                    class="sidebar-item flex items-center gap-3 px-4 py-2.5 text-[#c5d5e8] text-sm font-medium transition-all mx-2 rounded-lg mb-0.5 {{ $currentRoute === 'users' ? 'active' : '' }}">
                    <svg class="w-4.5 h-4.5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
    ...
    <div id="main-content" class="ml-[240px] min-h-screen flex flex-col transition-all duration-300">
            ...
            <div class="flex items-center gap-2">
                ...
                @unless (request()->routeIs('dashboard'))
                    <button onclick="history.back()"
                        class="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-gray-400 hover:text-gray-700 hover:bg-gray-100 transition-colors text-sm font-medium"
                        title="Kembali">
                ...
                @endunless
                {{-- Page Title --}}
                <span class="text-sm text-gray-500 font-medium hidden sm:block">@yield('title', 'Dashboard')</span>
            </div>

            {{-- User Dropdown --}}
            <div class="relative">
                <button onclick="toggleDropdown()"
                    class="flex items-center gap-2.5 hover:bg-gray-50 rounded-lg px-3 py-2 transition-colors focus:outline-none"
                    id="user-btn">
                    {{-- Avatar --}}
                    <div
                        class="w-8 h-8 rounded-full bg-[#162040] overflow-hidden flex items-center justify-center flex-shrink-0">
                ...
                <div id="user-dropdown"
                    ...
                    <div class="px-4 py-3 border-b border-gray-100">
                        <div class="flex items-center gap-3">
                            <div
                                class="w-10 h-10 rounded-full bg-[#162040] overflow-hidden flex items-center justify-center flex-shrink-0">
                                @if (auth()->user()->avatar_url)
                                    <img src="{{ auth()->user()->avatar_url }}" alt="Avatar"
                                        class="w-full h-full object-cover">
                                @else
                                    <span
                                        class="text-white text-sm font-semibold">{{ auth()->user()->initials }}</span>
                                @endif
                            </div>
                            <div class="min-w-0">
                                ...
                                <span
                                    class="inline-block mt-0.5 text-[10px] font-semibold px-1.5 py-0.5 rounded bg-amber-100 text-amber-700">
                    ...
                    <a href="{{ route('profile.show') }}"
                        class="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 hover:text-gray-800 transition-colors">
