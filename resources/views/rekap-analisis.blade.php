...
@section('content')
    ...
    <div id="tab-per-jenjang" class="hidden">
        ...
        {{-- Stats --}}
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6" id="jenjang-stats">
            <div class="bg-white rounded-xl p-4 shadow-sm border border-gray-100 text-center">
    ...
    <div id="tab-per-wilayah" class="hidden">
        ...
        <div class="bg-white rounded-xl p-5 shadow-sm border border-gray-100 mb-6">
            <h3 class="text-sm font-semibold text-[#162040] mb-4">Sebaran Sekolah per Provinsi</h3>
            <div class="relative" style="height: 220px;">
...
@push('scripts')
    <script>
        ...
        // ---- CHART: Bar – Sekolah & Guru per Jenjang ----
        ...
        function selectJenjang(j) {
            ...
            const raporData = [parseFloat(avg('literasi')), parseFloat(avg('numerasi')), parseFloat(avg('karakter'))];
            ...
            jenjangRaporChartInst = new Chart(document.getElementById('jenjangRaporChart'), {
                type: 'bar',
                data: {
                    labels: ['Literasi', 'Numerasi', 'Karakter'],
                    datasets: [{
        ...
        <tr class="hover:bg-gray-50/50 transition-colors">
            <td class="px-5 py-3 font-medium text-gray-800">${s.name}</td>
            <td class="px-3 py-3 text-gray-500 text-xs">${s.lokasi}</td>
            <td class="px-3 py-3 text-center text-gray-700 font-medium">${s.guru}</td>
